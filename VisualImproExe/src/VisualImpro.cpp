...
void interrupt_handler(int var)
{
  ...
  system("pkill nodejs");
  string rmcmd = string("ssh root@192.168.7.2 rm ") + string(WAVPATH) + string("*.wav");
  system(rmcmd.c_str());

  string logfile = string("logs/") + currentDateTime();

  string logcmd = string("scp root@192.168.7.2:") + string(LOGPATH) + string("log ") + logfile;
  system(logcmd.c_str());
...
-lancer VisualImpro
  ...
  for (auto file : files){
    string cmd = (string("scp ") + file + string(" root@192.168.7.2:") + string(WAVPATH));
    cout << cmd << endl;
    system(cmd.c_str());
  }
  ...
  string scpconfig = string("scp ") + string(CONFIGTMPFILE) + string(" root@192.168.7.2:") + string(CONFIGPATH);
  cout << scpconfig << endl;
  ...
  string execcmd = string("ssh root@192.168.7.2 ") + string("\'cd /root/Bela/projects/VisualImpro && ./VisualImpro config/") + string(CONFIGTMPFILE) + string("\' &");
  cout << execcmd << endl;
