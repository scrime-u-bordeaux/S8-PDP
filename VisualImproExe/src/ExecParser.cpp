...
ExecParser::ExecParser(std::string filein, std::string fileout) : _filein(filein), _fileout(fileout) {
  ...
  while (ifs){
	...
	lineout = std::string("FILE ") + std::string(WAVPATH) + relativePath(word);
	}
    }
...
std::list<std::string> ExecParser::getTracks(){
  return tracks;
}
