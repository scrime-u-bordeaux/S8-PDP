...
- window inclus un layout servant à afficher la matrice et un serveur servant a
communiquer avec bela.
-window sert uniquement à afficher la matrice et update en temps réel.
-quand bela envoie un update window print automatiquement.
-window herite de QMainWindow ce qui permettra des menus afin de, par exemple,
changer des parametres en route (exemple basique : les scale des colors).

- le layout principal contient uniquement une pixmap (en gros un gros tableau
de pixel, c'est une image mais qui a pas le meme nom) est qui quand on a besoin
de update les couleurs va dessiné des rectangle dessus pour chaque rect.
- c'est tres facile et leger comme methode car c'est juste une petite image.

-le server est un server tcp qui sert a communiquer avec bela.
-il se connecte avec bela et crée une socket, ensuite des que bela enverra
un paquet avec une matrice a l'interieur, le serveur retraduit le string en
matrice RGB
-ensuite  pour communiquer entre le layout et le server (emet un signal, c'est le truc technique de qt)
envoie la matrice qu'il a recu au layout.
