...
#define DLERROR(dlsymerror) dlsymerror = dlerror(); if (dlsymerror){cout << "Error : " << dlsymerror << endl;}
...
int main(int argc, char *argv[])
{
   ...
   cout << "************* VisualImpro ************" << endl;
	...
	if(configcolor != ""){dlcolor = configcolor;}
	...
	/* Calling Library */
	void * handle = dlopen("/root/Bela/projects/VisualImpro/process/libprocess.so", RTLD_LAZY);
	if (handle == NULL){
		cout << "error lib" << endl;
		return -1;}
	...
	for (auto file : files){
		...
		{	
			...
			cout << file << " n'est pas un fichier wav existant, il n'a pas été chargé" << endl;
		}
	}
	...
	if(gChSettings.nb_files + gChSettings.nb_analog + gChSettings.nb_audio > NB_TRACKS_MAX){	
	...
	ProcessMulti * p = new ProcessMultiCorrel(coeff1, color1, preproc1);
	gChSettings.proc = p;
	...
	while (1) {
		int c;
		if ((c = Bela_getopt_long(argv2.size()-1, argv2.data(), "h", customOptions, &settings)) < 0)
				break;
		switch (c) {
		case 'h':
	...
	logfile << "BUFFERSIZE : " << gChSettings.buffer_len << " SAMPLE_RATE : " << gChSettings.sample_factor * 22050 << std::endl;
	logfile.close();
