...
ProcessMultiCorrel::ProcessMultiCorrel(float (*coeffcorrel) (std::vector<float> s1, std::vector<float> s2), Triplet (*colorscale) (float coeff),   std::vector<std::vector<float> >(* preproc) (std::vector<std::vector<float> >)) : _coeffcorrel(coeffcorrel), _colorscale(colorscale), _preprocess(preproc)
{
...
vector<vector <Triplet> > ProcessMultiCorrel::create_matrix(std::vector<std::vector<float> > buffer){
...
void ProcessMultiCorrel::process(std::vector<std::vector<float> > buffer, Connection conn){
