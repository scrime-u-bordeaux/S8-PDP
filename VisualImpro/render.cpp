...
void effect(std::vector<std::vector <float> > & In, std::vector<std::vector <float> > & Out){
	...
	for (i = gUserSet.nb_audio; i < gUserSet.nb_audio + gUserSet.nb_analog ; i++){ //analog
	...
	for (i = gUserSet.nb_audio + gUserSet.nb_analog; i < gTotalTracks; i++){//files
		if (gUserSet.fileproc[i- gUserSet.nb_audio - gUserSet.nb_analog] == NULL){
			In[i].swap(Out[i]);
		}
		else{
			gUserSet.fileproc[i - gUserSet.nb_audio - gUserSet.nb_analog]->apply(In[i], Out[i]);
		}
	}
}

void processEffect(){
	genEffect(gEffectBufferInCopy, gIndIn, gEffectBufferOut, gIndOut, gCopySize, 2*gEffSize, effect); //from EffectManaging.hpp
	gEffectProcessing = 0;
	if(gEffectStart == 1){
...
int gFillPosition = -1; //last position that was filled. When gFillPosition = BUFFERLEN-1, buffer should be analyzed and empty'd
...
/******* MIXING TASKS AND VARS *******/

/* We will have here our code for mixing and vars */

//AuxiliaryTask gVolumeAdjustedTask;

void processVolume(){
	// Insert volume processing here
}
...
/********************************/
	...
	//Initialise effect buffers
	std::vector<std::vector <float> > effbuffer(gTotalTracks, std::vector<float>(2*gEffSize, 0.0f));
	std::vector<std::vector <float> > effbufferIn(gTotalTracks, std::vector<float>(gEffSize, 0.0f));
	gEffectBufferOut =  effbuffer;
	gEffectBufferIn = gEffectBufferInCopy = effbufferIn;

	//Initialise process buffers
	std::vector<std::vector<float> >procbuffer(gTotalTracks, std::vector<float>(gUserSet.buffer_len, 0.0f));
	...
	gProcessBufferCopy = procbuffer;
		...
		sampleStream[i] = new SampleStream(gUserSet.filenames[i].c_str(),nbchan,FILE_BUFFER_LEN);
	}
   	for (int i = 0; i<gUserSet.nb_files; i++){
	...
	cout << "* " << "Processing Speed (bufflen/samplerate) : " << speed  << " s" << endl;
	if(gUseEffects){
	...
	if( ( (gFillBuffersTask = Bela_createAuxiliaryTask(&fillBuffers, 80, "fill-buffer")) == 0 )
	|| ((gProcessBufferTask = Bela_createAuxiliaryTask(&processBuffer, 75, "process-buffer")) == 0 )
	|| ((gEffectTask = Bela_createAuxiliaryTask(&processEffect, 85, "apply-effect")) == 0 ) )
	// || ((gAdjustVolumeTask = Bela_createAuxiliaryTask(&processVolume, 90, "adjust-volume")) == 0 ) )
		return false;
	return true;
...
void render(BelaContext *context, void *userData)
{
	...
	int contextanalogFrames = context->analogFrames; //analogue channels are the reference
		...
		if(gUseEffects){ //uses additionnal buffers for effects
			...
			//files
		   	for(int i = 0; i<gNumStreams; i++){
		   		float sample = (sampleStream[i]->getSample(0) + sampleStream[i]->getSample(1))/2;
			...
			//audio
		  	for (int a = 0; a < gNumAudio; a++){
		    	...
		    	for(int s = 0; s<gNumStreams + gNumAnalog + gNumAudio; s++){ //transfert from Out to ProcessBuffer
			...
			for(int s = 0; s<gNumStreams; s++){
				float outsample;
			   	outsample = (sampleStream[s]->getSample(0)+sampleStream[s]->getSample(1))/2;
			       	gProcessBuffer[s][gFillPosition+1]= outsample;
				// Une idée serait d'écrire le son de chaque instrument sur la sortie audio
				// avec comme valeur la moyenne de ses coefficient de correlation comme ci-dessous
				// audioWrite(context, outsample, 0, moyCoeffCorrel);
				// Il faut cependant réfléchir à un moyen de récupérer ces valeurs de correlation
				out+=outsample;
		    	}
