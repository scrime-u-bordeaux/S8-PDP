 ...
 *  \date    July 2017
 *
 *  \brief  Object used to contain a wavefile.
 *
...
int SampleStream::openFile(const char *filename, int numChannels,
  ...
  gSampleBuf[0] =
      (SampleData *)calloc(bufferLength * numChannels, sizeof(SampleData));
  gSampleBuf[1] =
      (SampleData *)calloc(bufferLength * numChannels, sizeof(SampleData));
  ...
  if (gNumFramesInFile <= gBufferLength) {
    printf("Sample needs to be longer than buffer size. This example is "
           "intended to work with long samples.");
    return 1;
  }
  ...
  for (int ch = 0; ch < gNumChannels; ch++) {
    for (int i = 0; i < 2; i++) {
      ...
      if (getSamples(gFilename, gSampleBuf[i][ch].samples, ch, 0,
                     gBufferLength)) {
...
void SampleStream::processFrame() {
  ...
  if (gPlaying) {
    ...
    if (++gReadPtr >= gBufferLength) {
      ...
      //     rt_printf("Couldn't load buffer in time :( -- try increasing buffer
      //     size!");
...
float SampleStream::getSample(int channel) {
  if (gPlaying) {
    // Wrap channel index in case there are more audio output channels than the
    // file contains
    float out =
        gSampleBuf[gActiveBuffer][channel % gNumChannels].samples[gReadPtr];
    ...
    for (int ch = 0; ch < gNumChannels; ch++) {
      ...
      if (zeroPad) {
        ...
        for (int n = 0; n < numFramesToPad; n++)
          gSampleBuf[!gActiveBuffer]
                    [ch].samples[n + (gBufferLength - numFramesToPad)] = 0;
      }
    }
...
int SampleStream::bufferNeedsFilled() { return gBufferToBeFilled; }
...
void SampleStream::togglePlaybackWithFade(int toggle,
...
int SampleStream::isPlaying() { return gPlaying; }

// private libsndfile wrappers (previously in SampleLoader.h)

int SampleStream::getSamples(const char *file, float *buf, int channel,
  ...
  if (frameLen <= 0 || startFrame < 0 || endFrame <= 0 ||
      endFrame > sfinfo.frames) {
  ...
  int readcount =
      sf_read_float(sndfile, tempBuf, frameLen * numChannelsInFile); // FIXME
...
int SampleStream::getNumChannels(const char *file) { return sfinfo.channels; }

int SampleStream::getNumFrames(const char *file) { return sfinfo.frames; }
